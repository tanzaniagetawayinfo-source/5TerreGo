...
serve(async (req) => {
  ...
  if (godmode) requestBody.tools = [{ google_search: {} }];
  ...
  try {
    ...
    const grounding: Source[] = godmode ? (candidate?.groundingMetadata?.groundingChunks || []).map((chunk: Record<string, unknown>) => (chunk.web || {}) as Record<string, unknown>).map((web: Record<string, unknown>) => ({ title: text(web.title, 140) || "Web source", url: validHttpsUrl(web.uri), category: "web research", kind: "editorial" as const })).filter((source: Source) => Boolean(source.url)).slice(0, 6) : [];
