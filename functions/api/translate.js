...
export async function onRequestPost(context) {
  try {
    ...
    const upstream = await fetch('https://api.deepl.com/v2/translate', { method: 'POST', headers: { authorization: `DeepL-Auth-Key ${apiKey}`, 'content-type': 'application/json' }, body: JSON.stringify({ text, target_lang: target, preserve_formatting: true, formality: 'prefer_less' }) });
  ...
  } catch (error) { return json({ error: 'Translation failed.', details: error && error.message ? error.message : 'Unknown error' }, 502); }
}
